...
- **Seeding fixtures for a fresh/empty demo-site:** some tools list data that doesn't exist on a brand-new install (an A/B test project, a content type, a registered domain). Two patterns, pick by where the entity lives:
  ...
  - **CMS-native entities** (content types, documents, domains) that Engage only *reads* — use the chained CMS MCP via `mcpClientManager.callTool("cms", toolName, args)` from `../../../mcp-client.js` (see `content-types/__tests__/helpers/document-type-fixture.ts`). Notes:
