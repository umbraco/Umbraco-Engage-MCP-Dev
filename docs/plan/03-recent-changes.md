...
1. **Resolved, and this correction itself was wrong.** `cms-dev-mcp` pins `@anthropic-ai/claude-agent-sdk` to the exact version `0.2.39` (no caret) in its `package.json`, and its eval tests work. This project's `package.json` declared the same `^0.2.39` *range* — but the caret had let `npm install` drift the actually-installed version to `0.2.141`. The declared ranges looked identical; the installed versions weren't. The `TypeError: Object not disposable` crash *was* caused by that version drift after all — reproduced and then eliminated by reinstalling exactly `0.2.39`, with no other changes. Fixed by pinning this project's `package.json` to `"0.2.39"` exactly, matching `cms-dev-mcp`. See [Upgrades §1](02-upgrades.md#1-anthropic-aiclaude-agent-sdk--done-pinned-to-0239-fixing-the-eval-crash).
