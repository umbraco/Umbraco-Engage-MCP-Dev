...
This Engage MCP server is **functionally solid**: 146 tools across 37 real collections, live-verified OAuth auth against a real Umbraco+Engage instance, working CMS chaining (494 tools total in one session), and — as of this session — 63/64 integration test suites passing genuinely (not just locally-lucky). The core product works.

That said, suite-count measures *tests that exist*, not *coverage* — and coverage was the weakest part of this project by a wide margin: at one point this session, 68% of all tool files, and 93% of mutation tools specifically, were never exercised by any test at all. All 14 `delete-*` tools have since been brought to real coverage (calling the tool's own handler, not just seeding data via a builder), pulling mutation-tool coverage from 93% untested to 66% untested. The `post`/`put` side of that same gap is still open — see [Testing Improvements §4](01-testing-improvements.md#4-integration-test-coverage-is-critically-low--mutation-tools-especially) for the exact breakdown and what's left. This remains the single biggest risk in the project, bigger than any structural gap with `cms-dev-mcp` below.
...
4. **[Session changelog](04-session-changelog.md)** — what this session itself did to get from 37/49 passing integration tests (with 11 failures) to 63/64 (with all 14 `delete-*` tools now genuinely covered), kept as context for what "recent" means for *this* project going forward.
...
| Test *coverage* (not just file count) | **58% of tool files, 66% of mutation tools, never exercised by any test** (down from 68%/93% earlier this session — all `delete-*` tools now covered; `post`/`put` still open) | Not independently measured this pass — assume materially better given 456 files across the same-shaped tool surface | The real gap, bigger than the file-count ratio suggests — see [Testing Improvements §4](01-testing-improvements.md#4-integration-test-coverage-is-critically-low--mutation-tools-especially) |
...
2. **Testing improvements §4 — close the mutation-tool coverage gap.** This is the highest-*risk* item in the whole plan, not just the highest-friction one. All 14 `delete-*` tools (the highest-risk half of that gap) are now covered; the `post`/`put` half is still open — 37 of 56 mutation tools remain without test coverage of their own handler code.
