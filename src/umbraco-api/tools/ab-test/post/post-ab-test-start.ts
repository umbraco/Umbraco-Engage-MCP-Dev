...
const tool: ToolDefinition<typeof inputSchema.shape, typeof outputSchema> = {
  ...
  description:
    "Start, schedule, or reschedule a not-yet-stopped A/B test by setting its `startTime`. An A/B test's Draft/Scheduled/Running/Stopped/Completed status is computed from `startTime`/`endTime`/`isCompleted`, not stored directly, and there's no dedicated start/schedule endpoint - this tool fetches the test's current full state (the same data get-ab-test-view-model returns) and resubmits it through post-ab-test's own save endpoint with `startTime` changed and its page configuration rebuilt from `pageUnique`/`secondVariantPageUnique` (see their own descriptions for why those must be resupplied). Cannot restart an already-stopped test - Engage has no resume feature for a Stopped test, so this tool rejects that case with a clear error rather than silently no-opping. Returns a real error if `unique` doesn't match an existing test.",
  ...
  handler: async (params) => {
    ...
    if (test.endTime) {
      throw new ToolValidationError({
        ...
        status: 400,
        detail:
          "This A/B test already has an endTime (it was stopped or completed). Engage has no way to resume a stopped test - the backoffice's only available action on a Stopped test is 'Select winner', which finalizes it as Completed. Create a new test instead.",
      });
    }
